...
| **H-69** | Como **admin**, quiero **activar/desactivar el modo debug desde el panel de administración**, para **revisar logs detallados cuando hay un problema y silenciarlos en operación normal**. | 1. Switch "Modo Debug" en el panel admin (Settings > Debug Mode).<br>2. Al activarlo: todos los `console.log`/`console.debug` del backend se habilitan (incluyendo los de middleware de auth, WS, pagos).<br>3. Al desactivarlo: solo se muestran `console.warn` y `console.error` (errores y advertencias).<br>4. El estado persiste en MongoDB (colección `settings`) y se carga al iniciar el servidor.<br>5. Efecto inmediato — no requiere reinicio del servidor. |
